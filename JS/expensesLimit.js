...
const expensesLimitTableBody = document.querySelector(
  "#expenses-limit-table-body"
);
const totalLimitAmountDisplay = document.querySelector(
  "#expenses-limit-total-amount"
);
...
function updateExpensesLimitTable() {
  ...
  expensesLimit.forEach((expense) => {
    if (categoriesMap.has(expense.limitCategory)) {
      ...
      categoriesMap.set(
        expense.limitCategory,
        parseFloat(categoriesMap.get(expense.limitCategory)) +
          parseFloat(expense.limitAmount)
      );
    } else {
  ...
  categoriesMap.forEach((amount, category) => {
    ...
    limitDeleteButton.addEventListener("click", function () {
      expensesLimit = expensesLimit.filter(
        (expense) => expense.limitCategory !== category
      );
      updateExpensesLimitTable();
